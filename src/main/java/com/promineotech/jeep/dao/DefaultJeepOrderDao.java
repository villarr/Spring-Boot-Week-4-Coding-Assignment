...
public class DefaultJeepOrderDao implements JeepOrderDao{
	  ...
	  private NamedParameterJdbcTemplate jdbcTemplate;
		...
		@Override
		public Order saveOrder(Customer customer, Jeeps jeep, Color color, Engine engine, Tire tire, BigDecimal price,
				List<Option> options) {
	...
	/**
	   ...
	   * @return
	   */
	  private SqlParams generateInsertSql(Customer customer, Jeeps jeep, Color color,
	   ...
	   */
	  @Override
	  public Optional<Jeeps> fetchModel(JeepModel model, String trim, int doors) {
	    // @formatter:off
	    String sql = "" 
	        + "SELECT * " 
	        + "FROM models "
	  ...
	  class EngineResultSetExtractor implements ResultSetExtractor<Engine> {
	    ...
	    public Engine extractData(ResultSet rs) throws SQLException {
	      ...
	      return Engine.builder()
	          ...
	          .mpgHighway(rs.getFloat("mpg_hwy"))
	          .name(rs.getString("name"))
	          .price(rs.getBigDecimal("price"))
	   ...
	   *
	   */
	  class ModelResultSetExtractor implements ResultSetExtractor<Jeeps> {
	    @Override
	    public Jeeps extractData(ResultSet rs) throws SQLException {
	      rs.next();
